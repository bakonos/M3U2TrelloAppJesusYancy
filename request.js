...
const createTask = (task) => {
    ...
    taskDate.innerHTML = `<span class="card-task__date--tag-date">Fecha de entrega:</span> ${dateFormat(task.deadline)}`;

    let taskCreate = document.createElement("p");
    taskCreate.classList.add("card-task__date");
    taskCreate.innerHTML = `<span class="card-task__date--tag-date">Fecha de entrega:</span> ${dateFormat(task.created)}`;
